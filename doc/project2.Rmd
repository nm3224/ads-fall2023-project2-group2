...
```{r}
#calculating empirical distribution of total damge/total approved amount based on state
owner_data.ecdf <- ddply(owner_data, .(state), transform, ecdf=ecdf(totalDamage)(totalDamage))
p <- ggplot(owner_data.ecdf, aes(totalDamage, ecdf, colour = state)) + geom_line()
View(p)
```
